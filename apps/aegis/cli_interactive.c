...
int cmd_interactive(const char* project_root, const char* model, const char* resume_path) {
    ...
    if (resume_path) {
        ...
        if (aegis_session_load(resume_path, &loaded) == AEGIS_OK) {
            printf("resumed session %s with %zu messages\n", aegis_session_id(loaded), aegis_session_message_count(loaded));
    ...
    if (env_json && strcmp(env_json, "1") == 0) json_mode = 1;
    while (1) {
        printf("> ");
        fflush(stdout);
        ...
        if (strcmp(line, "/quit") == 0 || strcmp(line, "/exit") == 0 || strcmp(line, "/q") == 0) break;
        if (strncmp(line, "/clear", 6) == 0) { printf("\033[2J\033[H"); continue; }
        ...
        if (strcmp(line, "/tree") == 0) {
            aegis_session_t* sess = aegis_coding_agent_session(agent);
            printf("session %s branch %s parent %s messages %zu\n", aegis_session_id(sess), aegis_session_branch_id(sess), aegis_session_parent_id(sess) ? aegis_session_parent_id(sess) : "-", aegis_session_message_count(sess));
            continue;
        }
        if (strcmp(line, "/compact") == 0) { printf("compacting... (mock)\n"); continue; }
        if (strcmp(line, "/json") == 0) { json_mode = !json_mode; printf("json mode %s\n", json_mode ? "on" : "off"); continue; }
        if (json_mode) printf("{\"type\":\"user\",\"content\":\"");
        ...
        if (st2 == AEGIS_OK) {
            ...
            if (!json_mode) printf("\nDone.\n");
        } else if (st2 == AEGIS_ERR_CANCELLED) printf("cancelled\n");
...
int cmd_print(const char* prompt, const char* project_root, const char* model) {
    ...
    aegis_coding_agent_t* agent = NULL;
    aegis_status_t st = aegis_coding_agent_create(&cfg, &agent);
    if (st != AEGIS_OK) { fprintf(stderr, "failed to create agent: %s\n", aegis_status_str(st)); return 1; }
