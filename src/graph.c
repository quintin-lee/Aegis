...
static bool has_dependency(const aegis_task_graph_t* g,
                             uint32_t source, uint32_t target) {
    for (size_t i = 0; i < g->n_tasks; i++) {
        if (!g->tasks[i] || g->tasks[i]->id != target) continue;
...
static bool would_create_cycle(aegis_task_graph_t* g,
                                uint32_t source, uint32_t target) {
...
aegis_status_t aegis_task_graph_add_task(aegis_task_graph_t* graph,
                                          aegis_task_t* task) {
    if (!graph || !task) return AEGIS_ERR_INVALID;
...
aegis_status_t aegis_task_graph_remove_task(aegis_task_graph_t* graph,
    ...
    for (size_t i = 0; i < graph->n_tasks; i++) {
        for (size_t j = 0; j < graph->n_deps[i]; ) {
            if (graph->deps[i][j] &&
                (graph->deps[i][j]->source == task->id ||
                 graph->deps[i][j]->target == task->id)) {
                free(graph->deps[i][j]);
                graph->deps[i][j] = NULL;
...
aegis_status_t aegis_task_graph_add_dependency(aegis_task_graph_t* graph,
                                                aegis_task_t* source,
                                                aegis_task_t* target) {
    if (!graph || !source || !target) return AEGIS_ERR_INVALID;
    ...
    for (size_t i = 0; i < graph->n_tasks; i++) {
        if (graph->tasks[i] == source) source_found = true;
        if (graph->tasks[i] == target) target_found = true;
    }
    ...
    for (; tidx < graph->n_tasks; tidx++) {
        if (graph->tasks[tidx] == target) break;
    }
...
aegis_status_t aegis_task_graph_remove_dependency(aegis_task_graph_t* graph,
    ...
    for (size_t j = 0; j < graph->n_deps[tidx]; j++) {
        if (graph->deps[tidx][j] &&
            graph->deps[tidx][j]->source == source->id) {
...
aegis_status_t aegis_task_graph_ready_tasks(const aegis_task_graph_t* graph,
                                             ...
                                             size_t* out_count) {
    if (!graph || !out_vector || !out_count) return AEGIS_ERR_INVALID;
    ...
    for (size_t i = 0; i < graph->n_tasks; i++) {
        if (graph->tasks[i] &&
            graph->tasks[i]->state == AEGIS_TASK_READY) {
            count++;
        }
    }
...
aegis_status_t aegis_task_graph_validate(const aegis_task_graph_t* graph) {
    if (!graph) return AEGIS_ERR_INVALID;
    ...
    if (n > 0) {
        ...
        for (size_t i = 0; i < n; i++) {
            for (size_t j = 0; j < graph->n_deps[i]; j++) {
                ...
                for (size_t k = 0; k < n; k++) {
                    if (graph->tasks[k] &&
                        graph->tasks[k]->id == graph->deps[i][j]->source) {
                        in_degree[i]++;
                        break;
                    }
                }
        ...
        while (q_head < q_tail) {
            ...
            for (size_t i = 0; i < n; i++) {
                for (size_t j = 0; j < graph->n_deps[i]; j++) {
                    if (!graph->deps[i][j]) continue;
                    if (graph->tasks[curr] &&
                        graph->deps[i][j]->source == graph->tasks[curr]->id) {
                        in_degree[i]--;
                        if (in_degree[i] == 0) queue[q_tail++] = i;
                    }
                }
