...
static bool pattern_matches(const char* pattern, const char* name)
{
    ...
    if (strncmp(pattern, name, plen) != 0) return false;
    if (plen > 0 && pattern[plen - 1] == '*') return true;
    return nlen == plen;
}
...
const char* aegis_security_capabilities_str(aegis_capability_t caps)
{
    ...
    size_t off = 0;
    #define APPEND_CAP(bit, name) do { \
        if (caps & bit) { \
            if (off) off += (size_t)snprintf(buf + off, sizeof(buf) - off, ","); \
                                               ...
                                               aegis_capability_t required_caps)
{
    ...
    aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_POLICY_EVAL,
                                 AEGIS_SECURITY_AUDIT_INFO, required_caps,
                                 (int)(policy->n_rules - 1),
                                 "Added rule: pattern=%s caps=%s",
                                 r->pattern, aegis_security_capabilities_str(required_caps));
    return AEGIS_OK;
}
...
{
    if (!policy || idx >= policy->audit_count) return NULL;
...
{
    if (!policy || policy->audit_count == 0) return NULL;
...
aegis_status_t aegis_security_evaluate(aegis_security_policy_t* policy,
                                        ...
                                        const char* ctx)
{
    if (!policy || !tool_name) return AEGIS_ERR_INVALID;

    /* Phase 1: Capability check. */
    aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_CAP_CHECK,
                                 AEGIS_SECURITY_AUDIT_INFO, tool_caps, -1,
                                 "Tool '%s' declares capabilities=%s",
    ...
    if (granted) {
        aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_PERMISSION,
                                     AEGIS_SECURITY_AUDIT_ALLOW, tool_caps,
                                     matched_rule,
                                     "Permission GRANTED for tool '%s' (rule %d: %s)",
                                     tool_name, matched_rule,
                                     policy->rules[matched_rule].pattern);
        aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_DECISION,
                                     AEGIS_SECURITY_AUDIT_ALLOW, tool_caps,
                                     matched_rule,
                                     "Tool '%s' ALLOWED — all checks passed",
                                     tool_name);
        return AEGIS_OK;
    } else {
        aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_PERMISSION,
        ...
        aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_DECISION,
                                     AEGIS_SECURITY_AUDIT_DENY, tool_caps, -1,
                                     "Tool '%s' DENIED — capability check failed",
                                     tool_name);
...
bool aegis_security_has_permission(const aegis_security_policy_t* policy,
                                    ...
                                    aegis_capability_t tool_caps)
{
    return aegis_security_evaluate(
               (aegis_security_policy_t*)policy, tool_name, tool_caps, NULL) == AEGIS_OK;
}
    ...
    if (sandbox && sandbox->vtable && sandbox->vtable->pre_execute) {
        ...
        if (rc != AEGIS_OK) {
            aegis_security_audit_append(policy, AEGIS_SECURITY_AUDIT_SANDBOX,
                                         AEGIS_SECURITY_AUDIT_DENY, tool_caps, -1,
                                         "Sandbox pre_execute rejected: %s",
                                         aegis_status_str(rc));
        }
    }
    return rc;
