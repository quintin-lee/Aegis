...
aegis_task_state_t aegis_task_state(const aegis_task_t* task) {
    if (!task) return AEGIS_TASK_PENDING;
    aegis_mutex_lock(task->lock);
...
aegis_status_t aegis_task_set_input(aegis_task_t* task,
    ...
    if (!task) return AEGIS_ERR_INVALID;
    if (size > AEGIS_TASK_DATA_MAX) return AEGIS_ERR_INVALID;
...
const void* aegis_task_input(const aegis_task_t* task, size_t* out_size) {
    ...
    aegis_mutex_lock(task->lock);
    if (out_size) *out_size = task->input_size;
...
const void* aegis_task_output(const aegis_task_t* task, size_t* out_size) {
    ...
    aegis_mutex_lock(task->lock);
    if (out_size) *out_size = task->output_size;
...
aegis_status_t aegis_task_set_metadata(aegis_task_t* task,
    ...
    if (!task || !key) return AEGIS_ERR_INVALID;
    ...
    for (size_t i = 0; i < task->n_metadata; i++) {
        if (strcmp(task->metadata[i].key, key) == 0) {
            if (value) {
                strncpy(task->metadata[i].value, value,
                        sizeof(task->metadata[i].value) - 1);
    ...
    task->metadata[task->n_metadata].key[
        sizeof(task->metadata[task->n_metadata].key) - 1] = '\0';

    if (value) {
        strncpy(task->metadata[task->n_metadata].value, value,
                sizeof(task->metadata[task->n_metadata].value) - 1);
        task->metadata[task->n_metadata].value[
            sizeof(task->metadata[task->n_metadata].value) - 1] = '\0';
    } else {
...
const char* aegis_task_get_metadata(const aegis_task_t* task,
    ...
    for (size_t i = 0; i < task->n_metadata; i++) {
        if (strcmp(task->metadata[i].key, key) == 0) {
            const char* val = task->metadata[i].value[0] ?
                              task->metadata[i].value : NULL;
            aegis_mutex_unlock(task->lock);
            return val;
        }
    }
