...
{
    (void)user; (void)args; (void)token;
    return aegis_tool_result_set_string(out, "echo ok");
}

static void expect_ok(aegis_status_t rc, const char* msg)
{
    if (rc != AEGIS_OK) { fprintf(stderr, "FAIL %s: %d (%s)\n", msg, (int)rc, aegis_status_str(rc)); abort(); }
}
...
static void test_crash_recovery(void)
{
    ...
    d.name = "echo_step"; d.schema = k_no_schema; d.execute = echo_tool;
    ...
    printf("  checkpoint v=%u tasks=%zu state=%s goal=%s PASS\n",
           aegis_checkpoint_version(ckpt), aegis_checkpoint_task_count(ckpt),
           aegis_checkpoint_agent_state(ckpt), aegis_checkpoint_goal(ckpt));
    ...
    aegis_tool_def_t d2; memset(&d2,0,sizeof(d2));
    d2.name = "echo_step"; d2.schema = k_no_schema; d2.execute = echo_tool;
    ...
    printf("  second run rc=%d tasks=%u iter=%u recovered=%d\n", (int)rc, res2.tasks_executed, res2.iterations, res2.recovered_from_checkpoint);
