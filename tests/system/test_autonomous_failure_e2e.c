...
static void test_max_iterations(void){
    ...
    expect_ok(aegis_tool_registry_register(tr,&d),"reg");
    aegis_autonomous_agent_config_t cfg={.provider_registry=reg,.llm_provider_name=def.name,.tool_registry=tr,.max_iterations=1};
    aegis_autonomous_agent_t* aa=NULL; expect_ok(aegis_autonomous_agent_create(&aa,&cfg),"create");
    // use a plan that will not succeed, forcing max iterations
    aegis_autonomous_result_t res={0}; aegis_status_t rc=aegis_autonomous_agent_run(aa,"max iter goal",&res);
    printf("  rc=%d\n",(int)rc);
...
int main(void){test_retry_and_replan();test_max_iterations();printf("failure e2e PASS\n");return 0;}
