...
static void test_security_deny(void){
    ...
    aegis_tool_def_t d={0}; d.name="sec_step"; d.schema=k_no_schema; d.execute=ok_tool; d.capabilities=AEGIS_CAP_SHELL;
    expect_ok(aegis_tool_registry_register(tr,&d),"reg");
    aegis_security_policy_t* pol=NULL; expect_ok(aegis_security_policy_create(&pol),"pol");
    // no rule for SHELL, so default deny
    aegis_autonomous_agent_config_t cfg={.provider_registry=reg,.llm_provider_name=def.name,.tool_registry=tr,.security_policy=pol,.max_iterations=2};
    aegis_autonomous_agent_t* aa=NULL; expect_ok(aegis_autonomous_agent_create(&aa,&cfg),"create");
    aegis_autonomous_result_t res={0}; aegis_status_t rc=aegis_autonomous_agent_run(aa,"sec goal",&res);
...
static void test_security_allow(void){
    ...
    expect_ok(aegis_tool_registry_register(tr,&d),"reg");
    aegis_security_policy_t* pol=NULL; expect_ok(aegis_security_policy_create(&pol),"pol");
    expect_ok(aegis_security_policy_add_rule(pol,"*",AEGIS_CAP_SHELL|AEGIS_CAP_READ_FILE|AEGIS_CAP_WRITE_FILE|AEGIS_CAP_NETWORK|AEGIS_CAP_RUN_PROCESS|AEGIS_CAP_ACCESS_CRED),"add rule");
    aegis_autonomous_agent_config_t cfg={.provider_registry=reg,.llm_provider_name=def.name,.tool_registry=tr,.security_policy=pol,.max_iterations=2};
    aegis_autonomous_agent_t* aa=NULL; expect_ok(aegis_autonomous_agent_create(&aa,&cfg),"create");
    aegis_autonomous_result_t res={0}; aegis_status_t rc=aegis_autonomous_agent_run(aa,"sec goal",&res);
...
int main(void){test_security_deny();test_security_allow();printf("security e2e PASS\n");return 0;}
