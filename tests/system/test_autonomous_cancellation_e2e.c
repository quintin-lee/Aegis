...
static aegis_status_t slow_tool(void*u,const aegis_tool_args_t*a,const aegis_cancellation_token_t*t,aegis_tool_result_t*o){
    (void)u;(void)a;(void)o;
    for(int i=0;i<5;i++){ if(aegis_cancellation_token_is_cancelled(t)) return AEGIS_ERR_CANCELLED; struct timespec ts={0,10000000}; nanosleep(&ts,NULL); }
...
struct cancel_arg{ aegis_autonomous_agent_t* aa; };
static void* canceller(void* arg){ struct timespec ts={0,50000000}; nanosleep(&ts,NULL); aegis_autonomous_agent_cancel(((struct cancel_arg*)arg)->aa); return NULL; }
static void test_cancellation(void){
    ...
    aegis_tool_def_t d={0}; d.name="slow_step"; d.schema=k_no_schema; d.execute=slow_tool;
    expect_ok(aegis_tool_registry_register(tr,&d),"reg");
    aegis_autonomous_agent_config_t cfg={.provider_registry=reg,.llm_provider_name=def.name,.tool_registry=tr,.max_iterations=3};
    aegis_autonomous_agent_t* aa=NULL; expect_ok(aegis_autonomous_agent_create(&aa,&cfg),"create");
    struct cancel_arg ca={aa}; pthread_t th; pthread_create(&th,NULL,canceller,&ca);
    aegis_autonomous_result_t res={0}; aegis_status_t rc=aegis_autonomous_agent_run(aa,"cancel goal",&res);
...
int main(void){test_cancellation();printf("cancellation e2e PASS\n");return 0;}
