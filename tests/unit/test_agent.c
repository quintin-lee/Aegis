...
static void state_event_handler(const aegis_event_t* ev, void* ctx)
{
    ...
    if (aegis_event_type(ev) == 0x1000) { /* STATE_CHANGE */
        const aegis_agent_state_t* s = (const aegis_agent_state_t*)aegis_event_payload(ev)->data;
        if (s) {
            g_last_state = *s;
...
static void test_event_bus_subscription(void)
{
    ...
    aegis_event_bus_subscribe(aegis_agent_event_bus(agent), 0x1000, state_event_handler, NULL);

    assert(aegis_agent_init(agent) == AEGIS_OK);
    assert(g_state_events_received >= 2); /* INITIALIZING + READY */

    aegis_event_bus_unsubscribe(aegis_agent_event_bus(agent), 0x1000, state_event_handler, NULL);
