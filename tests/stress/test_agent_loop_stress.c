...
static void* thread_fn(void* arg) {
    (void)arg;
    // Each thread creates its own loop/session to avoid data race (loop is single-threaded by design)
...
int main(void) {
    ...
    for (int i = 0; i < N_THREADS; i++) pthread_create(&th[i], NULL, thread_fn, NULL);
    size_t total = 0;
