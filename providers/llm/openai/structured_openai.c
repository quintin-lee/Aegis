...
static int emit_record(sse_state_t* s, const char* record, size_t len)
{
    ...
    if (len < 5 || memcmp(record, "data:", 5) != 0) {
        return 1;
    }
    record += 5;
    len -= 5;
    while (len && (*record == ' ' || *record == '\t')) {
        ++record;
        --len;
    }
    if (len == 6 && memcmp(record, "[DONE]", 6) == 0) {
        ...
        if (s->callback(&ev, s->callback_user) != AEGIS_OK) {
            return 0;
        }
        s->saw_done = true;
        return 1;
    }
    if (len > OPENAI_MAX_RESPONSE || s->response_bytes > OPENAI_MAX_RESPONSE - len) {
        return 0;
    }
    ...
    if (!rcontent) {
        rcontent = json_string_after(json, "\"reasoning\"");
    }
    if (rcontent) {
    ...
    if (index_key) {
        ...
        if (json_uint_after(json, "\"index\"", &parsed_index)) {
            tool_index = parsed_index;
        }
    }
    if (tool_name || arguments) {
        ...
        if ((tool_name && !name_len) || (call_id && !id_len)) {
            free(json);
            return 0;
        }
        aegis_model_stream_event_t start = {.type      = AEGIS_MODEL_STREAM_TOOL_CALL_START,
                                            .index     = tool_index,
        ...
        if (!s->call_started[tool_index] && (tool_name || call_id)) {
            if (s->callback(&start, s->callback_user) != AEGIS_OK) {
                free(json);
                return 0;
            }
...
{
    if (!json || !out || len == 0 || len > OPENAI_MAX_RESPONSE) {
        return AEGIS_ERR_INVALID;
    }
    *out                             = NULL;
    ...
    if (status == AEGIS_OK) {
        status = aegis_message_set_content(message, decoded);
    }
    free(decoded);
    if (status != AEGIS_OK) {
